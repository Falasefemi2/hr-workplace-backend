...
interface CreateObjectiveInput {
  ...
  keyResults?: ReadonlyArray<KeyResultInput> // required for "flat", forbidden for "simplified"
  publish: boolean
}

export class OkrsService extends Context.Service<OkrsService>()("api/domain/okrs-service/OkrsService", {
  make: Effect.gen(function* () {
    ...
    const list = (organizationId: string, filters: { year?: number; quarter?: number }) =>
      repo.listObjectives(organizationId, filters).pipe(Effect.catchTag("DbError", Effect.orDie))
