 ...
 * @since 4.0.0
 */
export const layer = (api, options) =>
  HttpRouter.use(
 ...
 * @since 4.0.0
 */
export const group = (api, groupName, build) =>
  Layer.effectContext(
    Effect.gen(function* () {
 ...
 * @since 4.0.0
 */
export const endpoint = (api, groupName, endpointName, handler) =>
...
const getRequestContentType = (request) =>
...
function makeStreamEncoder(endpoint) {
  ...
  if (HttpApiSchema.isStreamUint8Array(streamSchema)) {
    return (response, context) => {
      ...
      return Effect.succeed(
        Response.stream(Stream.provideContext(response, context), {
          status,
          contentType,
        }),
      )
  ...
  return (response, context) => {
    ...
    return Effect.succeed(
      Response.stream(Stream.provideContext(encodeSseStream(response, sseEncoder), context), {
        status,
        contentType,
      }),
    )
